...
def extract_y_coordinates(megaNodes):
    y_coordinates = set()
    for row in megaNodes:
        for node in row:
            y_coordinates.add(node[1])
    return list(y_coordinates)
...
def find_intersection_points(lines, obstacle_polygon):
    intersection_points = []
    for line in lines:
        y = line  # y-coordinate of the line
        ...
        for i in range(len(obstacle_polygon)):
            ...
            if (y1 <= y <= y2) or (y2 <= y <= y1):
                ...
                intersection_points.append((intersect_x, y))
...
def insert_obstacle_paths(boustrophedon_path, obstacle_paths):
    new_path = []
    obstacle_start_positions = [path[0] for path in obstacle_paths]

    for i in range(len(boustrophedon_path) - 1):
        node1 = boustrophedon_path[i]
        node2 = boustrophedon_path[i + 1]

        new_path.append(node1)

        for obs_start, obstacle_path in zip(obstacle_start_positions, obstacle_paths):
            if node1[0] == node2[0] and obs_start[1] > node1[1] and obs_start[1] < node2[1]:
                new_path.append((node1[0], obs_start[1]))
                new_path.extend(obstacle_path[1:-1])
                new_path.append((node1[0], obs_start[1]))

        new_path.append(node2)

    new_path.append(boustrophedon_path[-1])
    return new_path
