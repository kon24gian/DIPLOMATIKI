...
def find_vertex_connecting_point(point, obstacle_polygon):
    for i in range(len(obstacle_polygon)):
        ...
        intersection = line_intersection((point, (point[0] + (p2[0] - p1[0]), point[1] + (p2[1] - p1[1]))), (p1, p2))
        if intersection and point_on_line_segment(intersection, p1, p2):
...
def point_on_line_segment(point, p1, p2):
    ...
    if abs(cross_product) > 1e-10:  # threshold to handle floating point inaccuracies
        return False
...
def find_vertices_on_perimeter(start_index, end_index, obstacle_polygon):
    vertices_cw = []
    vertices_ccw = []
    ...
    vertices_cw.append(obstacle_polygon[end_index])
    ...
    while index != end_index:
        ...
        index = (index - 1) % len(obstacle_polygon)
    vertices_ccw.append(obstacle_polygon[end_index])
...
def find_minimum_distance_path(pair, obstacle_polygon):
    ...
    start_vertex, start_index = find_vertex_connecting_point(start_point, obstacle_polygon)
    end_vertex, end_index = find_vertex_connecting_point(end_point, obstacle_polygon)

    # If either start_vertex or end_vertex wasn't found, return an empty path or handle it accordingly
    if start_vertex is None or end_vertex is None:
        return []

    # Create paths in both directions and calculate their lengths
    vertices_cw, vertices_ccw = find_vertices_on_perimeter(start_index, end_index, obstacle_polygon)
