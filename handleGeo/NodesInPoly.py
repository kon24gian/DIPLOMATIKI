...
import numpy as np
from handleGeo import InPolygon
from DARP.ConnectComponent import ConnectComponent
import math
...
class NodesInPoly(object):
    ...
    def strictlyInPoly(xMin, yMin, nodeDistance, hideInfo, nodeIntervalOffset, shiftX, shiftY, polygonCoordinates,
        ...
        subNodes = np.zeros((4 * xNodes, 4 * yNodes, 3))
        for i in range(xNodes):
            for j in range(yNodes):

                subNodes[4 * i][4 * j + 1][0] = megaNodes[i][j][0] - nodeIntervalOffset
                subNodes[4 * i][4 * j + 1][1] = megaNodes[i][j][1] + nodeIntervalOffset

                subNodes[4 * i + 1][4 * j + 1][0] = megaNodes[i][j][0] + nodeIntervalOffset
                subNodes[4 * i + 1][4 * j + 1][1] = megaNodes[i][j][1] + nodeIntervalOffset

                subNodes[4 * i][4 * j][0] = megaNodes[i][j][0] - nodeIntervalOffset
                subNodes[4 * i][4 * j][1] = megaNodes[i][j][1] - nodeIntervalOffset

                subNodes[4 * i + 1][4 * j][0] = megaNodes[i][j][0] + nodeIntervalOffset
                subNodes[4 * i + 1][4 * j][1] = megaNodes[i][j][1] - nodeIntervalOffset
                if InPolygon.check([subNodes[4 * i][4 * j + 1][0], subNodes[4 * i][4 * j + 1][1]],
                                   polygonCoordinates) and InPolygon.check(
                    [subNodes[4 * i + 1][4 * j + 1][0], subNodes[4 * i + 1][4 * j + 1][1]],
                    polygonCoordinates) and InPolygon.check(
                    [subNodes[4 * i][4 * j][0], subNodes[4 * i][4 * j][1]],
                    polygonCoordinates) and InPolygon.check(
                    [subNodes[4 * i + 1][4 * j][0], subNodes[4 * i + 1][4 * j][1]],
                    polygonCoordinates):
                ...
                subNodes[4 * i][4 * j + 1][2] = megaNodes[i][j][2]
                subNodes[4 * i + 1][4 * j + 1][2] = megaNodes[i][j][2]
                subNodes[4 * i][4 * j][2] = megaNodes[i][j][2]
                subNodes[4 * i + 1][4 * j][2] = megaNodes[i][j][2]
        ...
        if not hideInfo:
           print("Number of mega-nodes that are used for BNF: ", megaNodesCount)
           #print("Number of sub-nodes  ", 4.0 * megaNodesCount)
        ...
        if count_non_zero(cartObst.ravel()) != 0:
            for k in range(len(cartObst)):
                if InPolygon.check([subNodes[4 * i][4 * j + 1][0], subNodes[4 * i][4 * j + 1][1]],
                    ...
                    [subNodes[4 * i][4 * j][0], subNodes[4 * i][4 * j][1]],
                    cartObst[k]) or InPolygon.check(
                    [subNodes[4 * i + 1][4 * j][0], subNodes[4 * i + 1][4 * j][1]], cartObst[k]):
                    megaNodes[i][j][2] = 1
